...
- Gateway 验证规则版本升至 `1.20.0`，避免旧缓存复用变更前的分组和验证结果。
...
| 行业 | 财务四项完整/至少双源 | 严格市值两项完整/双源 | 财务金额覆盖率 | 市值金额覆盖率 | 门禁 |
| --- | ---: | ---: | --- | ---: | --- |
| 航空机场 | 13/13 | 13/13 | 100% | 100% | PASS |
| 航海装备Ⅱ | 10/10 | 10/10 | 100% | 100% | PASS |
| 证券Ⅱ | 48/49 | 49/49 | 基准营收 98.45%；最新营收 98.23%；两项利润 100% | 100% | PASS |

这里的“完整/双源”指 `usable=true` 且至少两个上游，不等于所有 Fact 都是
`verified`：存在单项 1%～5% 差异或部分官方输入缺失时，派生 Fact 会保持
`warning`。下游若声明只接受 `verified`，仍应按自己的发布门禁阻断这些公司，
不能把金额门禁 PASS 解释为 100% 无警告。

v1.20 对 CNINFO 解析做了以下通用修复，没有公司代码分支：

- 跳过只有分组标题、没有金额的首个字段命中，继续定位后续“合计”主表行；
- 拒绝把“调整前/影响金额/调整后”历史更正表识别为当期主表；
- 修复 PDF 文本层把小数或千分位拆成多个 token 的情况；
- 区分四列表格的“主体优先”和“期间优先”列顺序，并在 OCR 漏掉末尾母公司
  比较列时仍选择本期合并列与上期合并列；
- 扫描件默认渲染倍率从 3× 提升到 4×，同时规范化小数点后空格和常见中文
  OCR 混淆。长江证券 2024H1 页面因此从错误的 `831,658,317.39` 恢复为
  `3,831,659,317.39`，基准 TTM 营收重新可用；
- 行业审计命令现在先构建全 workspace，再加载 `dist`，防止源代码与运行产物
  版本错配生成虚假覆盖率。

最终唯一财务四项不完整的公司是方正证券：基准和最新 TTM 营收仍 fail-closed，
没有用东财与同花顺一致的聚合值覆盖官方后来比较口径。该缺口不会阻断行业金额
门禁，因为两个营收节点仍分别覆盖 98.45% 和 98.23% 的行业金额。

A 股财务事实的来源贡献如下：

- 航海装备四个财务节点均由 CNINFO、东方财富、同花顺覆盖 10/10 和 100%
  金额；航空机场的两个最新节点为三源 13/13，两个基准节点虽 CNINFO 为
  12/13，但东财与同花顺均为 13/13，因此最终双源覆盖仍为 100%；
- 证券的东财、同花顺均覆盖两个营收节点 48/49，金额分别为 98.45% 和
  98.23%，两项利润均覆盖 49/49 和 100%；
- 证券 CNINFO 分别覆盖基准营收 46/49（92.55%）、最新营收 45/49
  （93.08%）、基准利润 48/49（96.08%）、最新利润 46/49（94.46%）；
- 两个市值节点均由 CNINFO 的 point-in-time 总股本和腾讯历史收盘价覆盖，三
  个行业均为公司数与金额 100%。

v1.20 数据库中的 286 个营收/归母净利润 TTM Fact 全部使用 `ttm.flow.v1`，
其 3 项 `inputFactIds` 均能在持久化 facts 表中解释；缺失 derivation input 为 0，
`OFFICIAL_EXTRACTION_OUTLIER` 放行记录也为 0。最终审计产物为本地
`work/industry-audit-structured-v20-20260802.json`；`work/` 不进入 Git。
...
v1.15 曾增加不依赖公司名称的 `OFFICIAL_EXTRACTION_OUTLIER` 规则：
...
该规则表面上恢复了 10 个冲突输入 Fact，使证券从 44/49 提升到 49/49。复算数据库中
288 个营收/归母净利润 TTM Fact 全部带 `ttm.flow.v1` derivation，直接来源 TTM
为 0，说明它没有重新引入“累计值冒充 TTM”的问题，但不能证明选值正确。

反例是方正证券 2024 年报中的 2023 年营业收入比较值。CNINFO 原表清晰列出
`2024 年度 7,763,152,343.16`、`2023 年度 5,371,433,101.13`；东财与同花顺
却共同保留了另一口径约 71.19 亿元。这里官方值不是 OCR 或列选择错误，两个
聚合源一致也不能推翻它。因此 v1.16 删除了该覆盖路径，并新增反例测试：即使
官方值来自 PDF 且两个聚合源一致，只要差异超过 5% 仍输出
`OFFICIAL_OVERRIDE_SOURCE_CONFLICT`、`usable=false`。

被拒绝的诊断产物保存在本地
`work/industry-audit-structured-v15-20260802.json`；`work/` 不进入 Git，避免
把较大的应用侧验证数据和缓存提交到核心仓库。
...
- v1.15 的三行业 100% 结果已拒绝；v1.20 的可发布结论是航空 13/13、航海
  10/10、证券 48/49，且方正证券继续 fail-closed。
...
1. 将本轮冲突样例按“确定解析错误 / 真实比较口径 / 尚待裁决”分类后扩充
   golden corpus，逐步修复 CNINFO 列选择和单位识别；不能再用聚合源共识直接
   覆盖官方值。
