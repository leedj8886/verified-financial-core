...
- Gateway 验证规则版本升至 `1.16.0`，避免旧缓存复用变更前的分组和验证结果。
...
## 三行业全量复算（v1.15 诊断结果，已拒绝）

> **不要把本节的 49/49 作为发布结论。** v1.15 曾实验性允许两个聚合 API
> 一致时覆盖冲突的官方 PDF/OCR 提取值。随后核查方正证券官方原表发现，结构
> 完整的后来比较值也会被两个保留旧口径的聚合源同时反对，证明该规则不安全。
> v1.16 已恢复所有重大官方冲突 fail-closed。下表只保留为说明为何必须区分
> “解析错误”和“真实重述/比较口径”，最终覆盖率须以 v1.16 复算为准。
...
- v1.15 的三行业 100% 结果已拒绝；最终结论必须等待 v1.16 按官方冲突
  fail-closed 重新复算。
